...
if __name__=="__main__":
    ...
    #     print(f'train: {train_index}, val: {val_index}')
        ...
        drvfl = DeepRVFL(n_node, lam, w_range, b_range, n_layer)
        drvfl.train(X_val_train, y_val_train, n_class)
        prediction = drvfl.predict(X_val_test, False)
        acc = drvfl.eval(X_val_test, y_val_test)
    ...
    drvfl = DeepRVFL(n_node, lam, w_range, b_range, n_layer)
    drvfl.train(X_train, y_train, n_class)
    prediction = drvfl.predict(X_test, False)
    acc = drvfl.eval(X_test, y_test)
    print(f'\nTest accuracy: {acc}')